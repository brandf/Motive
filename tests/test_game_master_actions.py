...
def mock_game_master():
    ...
    with (
        ...
        # Use a regular dict instead of MagicMock to ensure __dict__ works properly
        mock_game_config = {
            'game_settings': {
                'num_rounds': 1,
            ...
            'rooms': {
                "start_room": {"id": "start_room", "name": "Starting Room", "description": "A simple starting room.", "exits": {"north": {"id": "north_exit", "name": "North", "destination_room_id": "other_room"}}, "objects": {"room_torch": {"id": "room_torch", "name": "room_torch", "object_type_id": "torch", "current_room_id": "start_room"}, "room_fountain": {"id": "room_fountain", "name": "Fountain", "object_type_id": "generic_fountain", "current_room_id": "start_room"}}},
...
def test_execute_effects_say_action(mock_game_master):
    ...
    assert any(e.message == f"{player_char.name} says: \"Hello everyone!\"." and
               "room_characters" in e.observers for e in events_generated)
