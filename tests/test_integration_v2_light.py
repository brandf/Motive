...
async def test_minimal_v2_light_room_characters(tmp_path):
    ...
    with llm_script({
        "Player_1": "> light \"Torch\"\n> light \"Rock\"\n> pass",
        "Player_2": "> pass",
    }):
        gm = GameMaster(config, game_id="it_light", deterministic=True, log_dir=str(tmp_path), no_file_logging=True)
        ...
        # p2 should observe the torch light event
        obs = gm.player_observations.get(p2.character.id, [])
        assert any("lights the" in ev.message and "Torch" in ev.message for ev in obs)
